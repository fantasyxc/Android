...
public class FavoriteActivity extends SliderActivity implements
	...
	class LoadFavoriteFromDb extends AsyncTask<Integer, Integer, Boolean> {
		...
		protected Boolean doInBackground(Integer... params) {
			...
			List<FavoriteEntity> favorites = mFavoriteDao.getFavorites(
					loadPage, PAGE_COUNT);
			if (favorites == null || favorites.size() == 0) {
				mHandler.obtainMessage(HANDLER_REFRESH, Boolean.valueOf(false))
						.sendToTarget();
			} else {
				mFavorites.addAll(favorites);
				mHandler.obtainMessage(HANDLER_REFRESH, Boolean.valueOf(true))
						.sendToTarget();
			}
