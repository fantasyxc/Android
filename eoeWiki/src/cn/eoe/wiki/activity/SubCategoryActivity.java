...
public class SubCategoryActivity extends CategoryActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent == null) {
			throw new NullPointerException(
					"Must give a CategoryChild in the intent");
		}
		...
		if (mParentCategory == null || TextUtils.isEmpty(mParentName)) {
			throw new NullPointerException(
					"Must give a CategoryChild and the parent name in the intent");
		}
	...
	protected void generateCategories(CategoryJson responseObject) {
		...
		if (categories != null) {
			for (CategoryChild category : categories) {
				...
				categoryLayout
						.setBackgroundResource(R.drawable.bg_stroke_grey_blue);
				mCategoryLayout.addView(categoryLayout);

				TextView tv = (TextView) mInflater.inflate(
						R.layout.category_title, null);
				tv.setText(category.getName());
				tv.setBackgroundResource(R.drawable.bg_nostroke_grey_blue_top);
				tv.setOnClickListener(new SubCategoryListener(category
						.getName(), mParentCategory.getName(), "", category
						.getUri(), SubCategoryActivity.this));
				...
				LayoutParams blankParams = new LayoutParams(
						LayoutParams.MATCH_PARENT, WikiUtil.dip2px(mContext, 8));
	...
	private void buildCategories(ViewGroup parent,
			List<CategoryChild> categories, int floot) {
		if (categories != null) {
			...
			for (int i = 0; i < size; i++) {
				...
				LayoutParams blankParams = new LayoutParams(
						LayoutParams.MATCH_PARENT, WikiUtil.dip2px(mContext, 1));
				...
				TextView tvChild = (TextView) mInflater.inflate(
						R.layout.category_item, null);
				if (floot > 1) {
					tvChild.setText("鈻�" + categorysChild.getName());
				} else {
					tvChild.setText(categorysChild.getName());
				}
				boolean notFinish = TextUtils.isEmpty(categorysChild
						.getPageID());
				if (notFinish) {
					tvChild.setTextColor(WikiUtil.getResourceColor(
							R.color.not_finish_color, mContext));
					tvChild.setOnClickListener(new SubCategoryNoFinishListener(
							SubCategoryActivity.this));
				} else {
					tvChild.setOnClickListener(new SubCategoryListener(
							categorysChild.getName(), mParentName,
							mParentCategory.getName(), categorysChild.getUri(),
							SubCategoryActivity.this));
				}
				tvChild.setPadding(WikiUtil.dip2px(mContext, 20) * floot,
						WikiUtil.dip2px(mContext, 5),
						WikiUtil.dip2px(mContext, 5),
						WikiUtil.dip2px(mContext, 5));
				parent.addView(tvChild);
				List<CategoryChild> subCategories = categorysChild
						.getChildren();
				if (i == (size - 1)
					...
					if (notFinish) {
						tvChild.setBackgroundColor(WikiUtil.getResourceColor(
								R.color.white, mContext));
					} else {
